    ...
    if (id) {
      ...
      if (row.owner_hash !== session.hash)
        throw new AdviceProblem(
          403,
          '이 공유를 관리할 수 없어요. 만든 브라우저에서 열어주세요.',
        );
      return adviceReply(
        await shareDetail(
          row,
          ...
          Math.max(
            1,
            Math.min(5, Number(new URL(r.url).searchParams.get('page')) || 1),
          ),
        ),
      );
    }
  ...
  try {
    ...
    if (b.action === 'create') {
      ...
      const count = await db
        .prepare(
          'SELECT COUNT(*) AS n FROM advice_shares WHERE owner_hash=? AND expires_at>?',
        )
        .bind(session.hash, new Date().toISOString())
        .first<{ n: number }>();
      if ((count?.n || 0) >= 30)
    ...
    if (b.action === 'delete') {
      await db.batch([
        db
          ...
          .bind(row.id),
        db
          .prepare('DELETE FROM advice_suggestions WHERE share_id=?')
          .bind(row.id),
        db
    ...
    if (b.action === 'close' || b.action === 'reopen') {
      ...
      await db
        .prepare(
          'UPDATE advice_shares SET status=? WHERE id=? AND owner_hash=?',
        )
    ...
    if (['adopt', 'unmark', 'hide'].includes(String(b.action))) {
      const s = await db
        .prepare(
          'SELECT id,status FROM advice_suggestions WHERE id=? AND share_id=?',
        )
      ...
      const updated = await db
        .prepare(
          'UPDATE advice_suggestions SET status=? WHERE id=? AND share_id=?',
        )
        .bind(
          b.action === 'adopt'
            ? 'adopted'
