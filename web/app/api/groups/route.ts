...
async function profile(r: Request) {
  ...
  return database()
    .prepare('SELECT id,nickname FROM profiles WHERE token_hash=?')
...
async function requireMember(groupId: string, userId: string) {
  ...
  if (!g)
    throw new Problem(
      403,
      '이 그룹에 접근할 수 없습니다. 초대 또는 참여 상태를 확인해 주세요.',
    );
  return g;
}
...
export async function POST(r: Request) {
  ...
  try {
    ...
    if (text.length > 60000)
      throw new Problem(413, '공유할 내용이 너무 큽니다.');
    ...
    if (!p && ['create', 'join'].includes(b.action)) {
      ...
      if (!nickname)
        throw new Problem(400, '그룹에서 사용할 이름을 입력해 주세요.');
      const token = random();
    ...
    if (b.action === 'preview') {
      ...
      if (!/^[a-f0-9]{48}$/.test(token))
        throw new Problem(404, '초대코드를 확인해 주세요.');
      const i = await db
        .prepare(
    ...
    if (!p)
      throw new Problem(401, '그룹을 만들거나 초대코드로 먼저 참여해 주세요.');
    if (b.action === 'create') {
      ...
      const count = await db
        ...
        .first<{ n: number }>();
      if ((count?.n || 0) >= 20)
        throw new Problem(400, '그룹은 20개까지 참여할 수 있어요.');
    ...
    if (b.action === 'invite') {
      if (g.ownerId !== p.id)
        throw new Problem(403, '그룹 관리자만 초대할 수 있어요.');
      const code = random(),
        expiresAt = new Date(Date.now() + 72 * 3600000).toISOString();
      await db.batch([
        db
          .prepare('UPDATE group_invitations SET revoked=1 WHERE group_id=?')
          .bind(id),
        db
          .prepare(
    ...
    if (b.action === 'revoke') {
      if (g.ownerId !== p.id)
        throw new Problem(403, '관리자만 초대를 취소할 수 있어요.');
      await db
        .prepare('UPDATE group_invitations SET revoked=1 WHERE group_id=?')
        .bind(id)
        .run();
    ...
    if (b.action === 'leave' || b.action === 'remove') {
      ...
      if (userId === g.ownerId)
        throw new Problem(400, '관리자는 그룹 삭제를 사용해 주세요.');
    ...
    if (b.action === 'deleteGroup') {
      if (g.ownerId !== p.id)
        throw new Problem(403, '관리자만 그룹을 삭제할 수 있어요.');
      await db.batch(
        [
    ...
    if (b.action === 'savePlan') {
      if (!validSharedPlan(b.plan))
        throw new Problem(400, '여행 이름·날짜·장소를 확인해 주세요.');
    ...
    if (b.action === 'deletePlan') {
      const plan = await db
        .prepare(
          'SELECT author_id AS authorId FROM group_plans WHERE id=? AND group_id=?',
        )
