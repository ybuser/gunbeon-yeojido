...
**본문은 매뉴얼 검증 결과이며 아래 문서 예제는 실제 데이터가 아니다.** 최초 작성 시 키가 없어 호출하지 않았지만, 공통 키 재설정 후2026-09-07 16:34 KST에 세 서비스 모두 실제 HTTP200/0000 및 표본1행을 확인했다. 최신 실호출 근거는 `reports/api_runtime_verification.json`이며 키·인증URL·관광정보 원문을 저장하지 않았다.

| 실제 표본 검증 | 요청 범위 | 반환 totalCount | 실제 수신 표본 |
|---|---|---:|---:|
| 집중률 | 철원51/51780, 기준일 응답20260907 | 1170 | 1 |
| 중심 관광지 | 철원51/51780, baseYm202607 | 100 | 1 |
| 연관 관광지 | 철원51/51780, baseYm202607 | 630 | 1 |

중심·연관의202608 요청은 정상0건이므로202607로 후퇴했다. 총5회 호출이며 아직 전체 페이지나 국문 contentid 매칭을 검증한 결과는 아니다. 아래의 미실행 문구는 매뉴얼 검토 당시의 설명이며 이 표본 검증 범위에서는 해소되었다.
