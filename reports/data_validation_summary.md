...
공통 `DATA_GO_KR_SERVICE_KEY` 재설정 후 2026-09-07 16:33 KST에 **국문 법정동5군×지정5유형25조합을 전부 페이징하여520건**을 확인했다. 철원114/화천81/양구69/인제107/고성149이며 수신 목록의 좌표 결측0, 호출37회, 오류0이다. 이번 범위의 유형은 관광지12/문화시설14/축제15/숙박32/음식점39로, 현실의 모든 시설이나 미선택 쇼핑·레포츠 등을 포괄한다는 의미가 아니다. 전용 무장애 지역목록의5군 대표 상세도 모두1행을 반환했다. `tour_api_live_validation.json`이 최신 국문 근거이며 이전237건은 구 코드 비교값으로만 보존한다. 선행 무장애 전체목록108건의 근거는 `accessibility_validation.json`의 `legal_dong_validation` 및 `summary`다.

추가 관광API3종도 같은 공통 키로 실제 HTTP200/정상코드0000 및 최소1행을 확인했다(총5회). 철원 집중률은 `baseYmd=20260907`, totalCount1170 중1행을 표본 수신했다. 중심·연관은 `baseYm=202608`이 정상0건이어서 `202607`로 후퇴하여 각각 totalCount100/630 중1행을 수신했다. 이는 전체 페이지 수집 통계가 아닌 **키 접근과 표본 가용성 검증**이다. `api_runtime_verification.json`에 파라미터·결과코드·필드·건수만 저장했다. 키·키 포함 URL·관광API 원문은 저장하지 않았다. 매뉴얼 검토 직후 키 파일 부재로0회 종료했던 시도는 해결된 이력이며 현재 상태가 아니다. API 테스트 성공과 실제 앱 화면 연동 완료는 구분한다.
